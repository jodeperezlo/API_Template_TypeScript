...
app.get(`${api}`, (req: Request, res: Response) => {
  res.send(
    `<h1>Bienvenido a la API REST</h1> <p>Access to: <a href="${config.API_PROTOCOL}://${
      config.API_HOST
    }:${app.get("port")}${config.API_URL}/swagger">API REST Docs</a> for more information.</p>`
  );
});
...
// Swagger Docs
app.use(`${api}/swagger`, swaggerUI.serve, swaggerUI.setup(swaggerJsDoc(swaggerOptions)));
